...
export async function analyzeOmrImage(
  ...
  results.forEach((result) => {
    ...
    const marked = top.percent >= Math.max(MARKED_THRESHOLD, baselineThreshold);
    const gapOk = !runner || top.percent - runner.percent >= Math.max(UNSURE_DELTA, stddev * 0.25);

    if (marked && gapOk) {
      result.detected = top.letter;
    } else if (top.percent >= UNSURE_THRESHOLD) {
      result.detected = null;
    } else {
      result.detected = null;
    }
